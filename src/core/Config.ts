...
export const CONFIG = {
  ...
  fire: {
    ...
    thermalForce: 13,
    /**
     * Потолок суммарного подброса — в долях от того, чем дрон может ему
     * ответить (maxClimbSpeed × climbResponse).
     *
     * Потоки соседних очагов складываются, а у ворот их три штуки в трёх
     * метрах друг от друга: без потолка сумма доходила до 26 м/с² и
     * пересиливала снижение — на полном «вниз» дрон продолжало тянуть вверх.
     *
     * Доля, а не абсолютная величина, потому что порог зависит от резвости
     * борта: стоит поменять скороподъёмность в панели настройки, и жёстко
     * заданные м/с² снова начнут либо запирать дрон, либо не ощущаться.
     * При 0.55 полное «вниз» даёт снижение примерно 0.45 × maxClimbSpeed,
     * а на отпущенном стике поток уносит вверх примерно на 0.55 от неё.
     */
    thermalLiftRatio: 0.55,
...
export const TUNABLES: TunableMeta[] = [
  ...
  { group: 'Ветер', path: 'fire.thermalLiftRatio', label: 'Потолок подброса (доля)', min: 0, max: 1.5, step: 0.05 },
