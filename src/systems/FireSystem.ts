...
export class FireSystem {
  ...
  getUpdraft(position: THREE.Vector3, out: THREE.Vector3): void {
    ...
    // Потолок на суммарный подброс. Очаги у ворот стоят вплотную, их колонны
    // складывались и перевешивали снижение: пилот жал «вниз», а дрон уходил
    // вверх. Считаем от того, чем дрон может ответить, — так порог остаётся
    // верным и после правки скороподъёмности в панели настройки.
    // Тянуть вниз поток по-прежнему может без ограничений: падение в огонь
    // ничем хорошим кончаться и не должно.
    const maxLift = cfg.flight.maxClimbSpeed * cfg.flight.climbResponse * f.thermalLiftRatio;
    if (out.y > maxLift) out.y = maxLift;
  }
